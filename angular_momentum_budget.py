...
R_gas = 8.3143
# Gas constant
heights = [0.,  0.05,  0.2,  0.4,  0.8,  1.3,  2.2,  3.3,  4.7,  6.5,  8.6,
...
class Simulation:
    ...
    def area_weights(self):
        ...
        if self.data['aire'] is not None:
            self.areas = self.data['aire']
        else:
...
class AngularMomentumBudget(Simulation):
    """ Zonal component of axial angular momentum budget in spherical coordinates
        ...
        Resulting angular momentum terms have dimension (model level, latitude) 
        There is an option to vertically mean sections of the atmosphere """
